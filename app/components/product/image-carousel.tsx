...
const ImageCarousel = ({ data, activeIndex = 0 }: CarouselProps) => {
  const ref = React.useRef<ICarouselInstance>(null);
  const progress = useSharedValue<number>(0);
  const isFirstRender = useRef(true);

  const safeIndex =
    activeIndex >= 0 && activeIndex < data.length ? activeIndex : 0;
  ...
  useEffect(() => {
    ...
    ref.current?.scrollTo({ index: safeIndex, animated: true });
  }, [safeIndex]);
