...
const PaymentStep = ({
  ...
  const isManualPaymentSelected = selectedProviderId === 'pp_system_default';

  const { data: paymentAccounts, isLoading: isLoadingPaymentAccounts } =
    useSellerPaymentDetails(cart?.id, isManualPaymentSelected);
  ...
  useEffect(() => {
    ...
    onSelectProvider(paymentProviders[0].id);
  }, [
    paymentProviders,
    isLoadingProviders,
    selectedProviderId,
