...
import { findGuestOrderById } from '@utils/guest-orders';
  ...
  const { data: order, isPending } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Guest orders aren't retrievable from this endpoint until the
        // order is transferred to an account; fall back to the locally
        // cached copy saved at checkout time.
        const cachedOrder = await findGuestOrderById(orderId);
        if (cachedOrder) {
          return cachedOrder;
        }
        throw err;
