...
const SignIn = ({ route }: SignInProps) => {
  ...
  const onSubmit = async (data: SignInSchema) => {
    try {
      Keyboard.dismiss();
      setLoading(true);
      await login(phoneToAccountEmail(data.phone), data.password);

      if (redirectTo === 'Orders' || redirectTo === 'Checkout') {
        navigation.dispatch(
          CommonActions.reset({
            index: 1,
            routes: [{ name: 'Main' }, { name: redirectTo }],
          }),
        );
        return;
      }
